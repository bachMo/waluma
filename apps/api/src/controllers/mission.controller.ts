...
export async function creerMission(req: AuthRequest, res: Response): Promise<void> {
  ...
  if (type === 'IMMEDIATE') {
    const praticien = await findBestPraticien({
      specialite,
      latitude,
      longitude,
      urgence,
...
export async function updateStatutMission(req: AuthRequest, res: Response): Promise<void> {
  ...
  const timestampField: Record<string, string> = {
    ...
    EN_ROUTE: 'enRouteAt',
    ARRIVE: 'arriveeAt',
    EN_COURS: 'debutSoinAt',
    TERMINEE: 'finSoinAt',
    ANNULEE: 'annuleeAt',
  }

  const updateData: Record<string, unknown> = { statut }
  if (timestampField[statut]) {
    updateData[timestampField[statut]] = new Date()
  }
...
export async function assignerPraticien(req: AuthRequest, res: Response): Promise<void> {
  ...
  if (!praticienId) {
    res.status(400).json({ error: 'praticienId requis' })
    return
  }
...
export async function soumettreCompteRendu(req: AuthRequest, res: Response): Promise<void> {
  const { id } = req.params
  const {
    acteRealise, description,
    tension, temperature, pouls, spo2,
    recommandations, suiteNecessaire,
    photoUrls = [],
    signaturePatientUrl,
  } = req.body
...
export async function noterMission(req: AuthRequest, res: Response): Promise<void> {
  ...
  if (!mission || !mission.praticienId) {
    res.status(404).json({ error: 'Mission introuvable ou sans praticien' })
    return
  }
  ...
  const avis = await prisma.avis.create({
    data: {
      missionId: id,
      patientId: req.user!.userId,
      praticienId: mission.praticienId,
      note,
      commentaire,
      tags,
    },
  ...
  await prisma.praticien.update({
    where: { id: mission.praticienId },
    data: { noteMoyenne: avgResult._avg.note },
  })
