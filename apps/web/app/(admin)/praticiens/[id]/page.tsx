...
interface MissionImpayee {
  id: string; specialite: string; montantTotal: number; finSoinAt: string
...
export default function PraticienDetailPage() {
  ...
  async function load() {
    ...
    try {
      ...
      // Charger les missions impayées
      try {
        const { data: missions } = await api.get(`/missions?praticienId=${id}&statut=TERMINEE&limit=50`)
        const impayees = (missions.missions || []).filter((m: { paiement?: { statut: string } | null }) =>
          !m.paiements?.[0] || m.paiements?.[0].statut !== 'PAYE'
        )
  ...
  async function handleToggleBlocage() {
    ...
    const msg = newBloque
      ? 'Ce praticien ne recevra plus de nouvelles missions. Confirmer le blocage ?'
  ...
  return (
    <div className="flex flex-col h-full">
      <Topbar title="Fiche praticien">
        <button onClick={() => router.push('/praticiens')} className="text-xs font-semibold border border-gray-200 rounded-lg px-3 py-1.5 hover:bg-gray-50 transition">← Retour</button>
        {activeTab === 'infos' && (
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        <div className="flex items-start justify-between mb-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="font-extrabold text-xl text-gray-900 tracking-tight">{praticien.user.prenom} {praticien.user.nom}</h1>
              <div className="flex items-center gap-3 mt-1 flex-wrap">
                ...
                {praticien.bloque && (
                  <span className="text-xs font-bold px-2 py-0.5 rounded-full bg-orange-100 text-orange-700">🔒 Missions bloquées</span>
                )}
          ...
          <div className="flex items-center gap-2 flex-wrap justify-end">
            ...
            >
              {togglingBlocage ? '...' : praticien.bloque ? '🔓 Débloquer les missions' : '🔒 Bloquer les missions'}
            </button>
          </div>
        </div>
        ...
        {missionsImpayees.length > 0 && (
          <div className="bg-orange-50 border border-orange-200 rounded-xl px-4 py-3 flex items-center gap-2 mb-5 text-sm text-orange-700">
            <span>💰</span>
            <span>
              <strong>{missionsImpayees.length} mission{missionsImpayees.length > 1 ? 's' : ''} non payée{missionsImpayees.length > 1 ? 's' : ''}</strong> — ce praticien est automatiquement exclu du matching.
              <button onClick={() => setActiveTab('paiements')} className="ml-2 underline font-semibold">Voir les détails</button>
            </span>
          </div>
        )}
        ...
        <div className="flex gap-1 mb-5 border-b border-gray-100">
          {[
            ...
            { key: 'paiements', label: `Paiements en attente (${missionsImpayees.length})`, alert: missionsImpayees.length > 0 },
          ].map(tab => (
            <button key={tab.key} onClick={() => setActiveTab(tab.key as typeof activeTab)}
              className={`text-sm font-semibold px-4 py-2.5 border-b-2 transition flex items-center gap-1.5 ${activeTab === tab.key ? 'border-[#0d5068] text-[#0d5068]' : 'border-transparent text-gray-400 hover:text-gray-600'}`}>
              {tab.label}
                      ...
                      <tr key={doc.id} className="border-b border-gray-50 hover:bg-gray-50 transition">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-1.5">
                            {doc.statut !== 'VALIDE' && (
                              <button onClick={() => handleDocStatut(doc.id, 'VALIDE')} className="text-[11px] font-bold bg-green-50 text-green-700 border border-green-200 rounded-lg px-2 py-1 hover:bg-green-100 transition">Valider</button>
                            )}
                            {doc.statut !== 'REFUSE' && (
                              <button onClick={() => handleDocStatut(doc.id, 'REFUSE')} className="text-[11px] font-bold bg-red-50 text-red-700 border border-red-200 rounded-lg px-2 py-1 hover:bg-red-100 transition">Refuser</button>
                            )}
        ...
        {activeTab === 'paiements' && (
          <div className="space-y-4">
            <div className="bg-orange-50 border border-orange-200 rounded-xl p-4 text-sm text-orange-700">
              <strong>Pourquoi ce praticien est bloqué du matching ?</strong>
              <p className="mt-1 text-orange-600">Tant qu&apos;il a des missions terminées non payées, il est automatiquement exclu des propositions de nouvelles missions. Seul un admin peut débloquer manuellement.</p>
            </div>
            ...
            {missionsImpayees.length === 0 ? (
              ...
              <div className="bg-white border border-gray-100 rounded-xl overflow-hidden">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {missionsImpayees.map((m: MissionImpayee) => (
                      <tr key={m.id} className="border-b border-gray-50 hover:bg-gray-50 transition">
                        ...
                        <td className="px-4 py-3">
                          <a href={`/missions/${m.id}`} className="text-[11px] font-bold text-[#0d5068] hover:underline">Voir la mission</a>
                        </td>
                      </tr>
                    ))}
                  </tbody>
            ...
            {praticien.bloque && missionsImpayees.length > 0 && (
              <div className="flex justify-end">
                <button
                  onClick={handleToggleBlocage}
                  disabled={togglingBlocage}
                  className="text-xs font-bold bg-green-50 text-green-700 border border-green-200 rounded-lg px-4 py-2 hover:bg-green-100 transition disabled:opacity-50"
                >
