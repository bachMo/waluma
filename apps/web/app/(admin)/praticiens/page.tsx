  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white rounded-2xl p-6 shadow-2xl max-w-sm w-full mx-4">
        <div className={`w-12 h-12 rounded-xl flex items-center justify-center mb-4 ${danger ? 'bg-red-50' : 'bg-orange-50'}`}>
          <span className="text-xl">{danger ? '🗑' : '🔒'}</span>
        </div>
        <h3 className="font-bold text-gray-900 text-base mb-2">Confirmation</h3>
        <p className="text-sm text-gray-500 mb-6">{message}</p>
        <div className="flex gap-3">
          <button onClick={onCancel} className="flex-1 text-sm font-semibold border border-gray-200 rounded-xl py-2.5 hover:bg-gray-50 transition">Annuler</button>
          <button onClick={onConfirm} className={`flex-1 text-sm font-bold text-white rounded-xl py-2.5 transition ${danger ? 'bg-red-600 hover:bg-red-700' : 'bg-orange-500 hover:bg-orange-600'}`}>Confirmer</button>
        </div>
      </div>
    </div>
  )
...
function ActionMenu({ praticien, onAction }: { praticien: Praticien; onAction: (action: string, id: string) => void }) {
  ...
  // Blocage/déblocage missions
  if (praticien.bloque) {
    actions.push({ key: 'DEBLOQUER', label: 'Débloquer les missions', color: 'text-green-700' })
  } else {
    actions.push({ key: 'BLOQUER', label: 'Bloquer les missions', color: 'text-orange-600' })
  }
  actions.push({ key: 'DELETE', label: 'Supprimer', color: 'text-red-700 font-bold' })

  return (
    <div ref={ref} className="relative">
      <button onClick={e => { e.stopPropagation(); setOpen(!open) }} className="w-7 h-7 flex flex-col items-center justify-center gap-[3px] rounded-lg hover:bg-gray-100 transition">
        {[0, 1, 2].map(i => <div key={i} className="w-1 h-1 rounded-full bg-gray-400" />)}
      </button>
      {open && (
        <div className="absolute right-0 top-8 bg-white border border-gray-100 rounded-xl shadow-lg z-10 py-1 min-w-[180px]">
          <Link href={`/praticiens/${praticien.id}`} className="block px-3 py-2 text-[12px] text-gray-700 hover:bg-gray-50 transition" onClick={() => setOpen(false)}>
            Voir la fiche
          </Link>
          <div className="border-t border-gray-50 my-1" />
          {actions.map(a => (
            <button key={a.key} onClick={e => { e.stopPropagation(); setOpen(false); onAction(a.key, praticien.id) }}
...
export default function PraticiensPage() {
  ...
  async function handleAction(action: string, id: string) {
    ...
    if (action === 'BLOQUER') {
      setConfirmModal({
        message: 'Ce praticien ne recevra plus de nouvelles missions jusqu\'au déblocage. Confirmer ?',
        danger: false,
        onConfirm: async () => {
          setConfirmModal(null)
          await api.patch(`/praticiens/${id}/bloquer`, { bloque: true })
          load()
        },
      })
      return
    ...
    if (action === 'DEBLOQUER') {
      setConfirmModal({
        message: 'Ce praticien pourra à nouveau recevoir des missions. Confirmer le déblocage ?',
        danger: false,
        onConfirm: async () => {
          setConfirmModal(null)
          await api.patch(`/praticiens/${id}/bloquer`, { bloque: false })
          load()
        },
      })
      return
    ...
    if (groupeAction === 'DELETE') {
      setConfirmModal({
        ...
        onConfirm: async () => {
          ...
          setSelected([]); setGroupeAction(''); setApplyingGroupe(false); load()
        },
      })
      return
    }
  ...
  const nbBloques = praticiens.filter(p => p.bloque).length

  return (
    <div className="flex flex-col h-full">
      <Topbar title="Praticiens">
        <Link href="/praticiens/ajouter" className="flex items-center gap-1.5 text-xs font-bold bg-[#22c55e] text-white rounded-lg px-3 py-1.5 hover:bg-[#16a34a] transition">
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        {/* Alertes */}
        {nbEnAttente > 0 && (
          <div className="bg-yellow-50 border border-yellow-200 rounded-xl px-4 py-3 flex items-center gap-2 mb-3 text-sm text-yellow-700">
            <span>⏱</span>
        ...
        {nbBloques > 0 && (
          <div className="bg-orange-50 border border-orange-200 rounded-xl px-4 py-3 flex items-center gap-2 mb-5 text-sm text-orange-700">
            <span>🔒</span>
            <span><strong>{nbBloques} praticien{nbBloques > 1 ? 's' : ''}</strong> bloqué{nbBloques > 1 ? 's' : ''} (paiements en attente).</span>
          </div>
        )}
        ...
        <div className="flex items-center gap-3 mb-4 flex-wrap">
          ...
          {selected.length > 0 && (
            <div className="flex items-center gap-2 ml-auto bg-[#0d5068]/5 border border-[#0d5068]/20 rounded-lg px-3 py-1.5">
              ...
              <select value={groupeAction} onChange={e => setGroupeAction(e.target.value)}
              ...
              <button onClick={handleGroupeAction} disabled={!groupeAction || applyingGroupe}
        ...
        <div className="bg-white border border-gray-100 rounded-xl overflow-hidden">
          <table className="w-full text-sm" style={{ tableLayout: 'fixed' }}>
            <colgroup>
              <col style={{ width: '3%' }} /><col style={{ width: '20%' }} /><col style={{ width: '14%' }} />
            ...
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100">
                ...
                {['Praticien', 'Spécialité', 'Statut', 'Missions', 'Note', 'Disponible', 'Bloqué', 'Docs', 'Ajouté le', ''].map(h => (
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={11} className="text-center py-10 text-gray-400 text-sm">Chargement...</td></tr>
              ) : filtered.length === 0 ? (
                <tr><td colSpan={11} className="text-center py-10 text-gray-400 text-sm">Aucun praticien trouvé</td></tr>
              ) : filtered.map(p => (
                <tr key={p.id} onClick={() => router.push(`/praticiens/${p.id}`)}
                  className={`border-b border-gray-50 hover:bg-gray-50 transition cursor-pointer ${
                    selected.includes(p.id) ? 'bg-blue-50/30' :
                    p.bloque ? 'bg-orange-50/40' :
                ...
                >
                  ...
                  <td className="px-3 py-3 text-center">
                    {p.bloque ? (
                      <span className="text-[11px] font-bold px-2 py-0.5 rounded-full bg-orange-100 text-orange-700">🔒 Bloqué</span>
                    ) : (
