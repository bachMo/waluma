...
function Toggle({ value, onChange }: { value: boolean; onChange: (v: boolean) => void }) {
  return (
    <button
      ...
      className={`w-10 h-5 rounded-full relative transition-colors flex-shrink-0 ${value ? 'bg-[#22c55e]' : 'bg-gray-200'}`}
    >
      <div className={`w-3.5 h-3.5 bg-white rounded-full absolute top-0.5 transition-transform ${value ? 'translate-x-5' : 'translate-x-0.5'}`}></div>
    </button>
  )
}

function ParamRow({ label, sub, children }: { label: string; sub?: string; children: React.ReactNode }) {
  return (
...
export default function ParametresPage() {
    ...
    async function load() {
      try {
        ...
        Object.values(data.grouped as GroupedParams).forEach((cat) => {
          Object.entries(cat as Record<string, ParamValue>).forEach(([cle, p]) => {
            flat[cle] = p.valeur
          })
        })
        setLocalValues(flat)
  ...
  function setValue(cle: string, val: number | boolean | string) {
    setLocalValues(prev => ({ ...prev, [cle]: val }))
  }
...
function numInput(cle: string, unit: string) {
  return (
    <div className="flex items-center gap-2">
      <input
        type="number"
        value={localValues[cle] !== undefined && !isNaN(Number(localValues[cle])) ? Number(localValues[cle]) : ''}
        onChange={e => setValue(cle, parseFloat(e.target.value) || 0)}
  ...
  return (
    <div className="flex flex-col h-full">
      <Topbar title="Paramètres">
        {saved && (
          <span className="text-xs font-semibold text-green-600 bg-green-50 border border-green-200 rounded-lg px-3 py-1.5">
            ✓ Enregistré
          </span>
        )}
        <button
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        <div className="grid grid-cols-2 gap-5">
          ...
          <div className="bg-white border border-gray-100 rounded-xl p-5">
            ...
            <ParamRow label="Commission Waluma standard" sub="Prélevée à la clôture (%)">
            ...
            <ParamRow label="Frais de déplacement" sub="Ajoutés automatiquement">
          ...
          <div className="bg-white border border-gray-100 rounded-xl p-5">
            ...
            <ParamRow label="Notifications push" sub="Changements de statut des missions">
